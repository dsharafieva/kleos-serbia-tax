...
Three regimes as tabs, gross-to-net or net-to-gross. Net-to-gross is solved numerically, so it stays correct for the freelancer model where the relationship is piecewise-linear.

### Checklist

Clicking a row toggles it and the note underneath recomputes. Four bands:

| Ticked | What the note says | How it looks |
|---|---|---|
| 0 | Reads as a genuine contractor relationship | default |
| 1–2 | Still reads as contracting; keep the contract matching reality and check the income share | default |
| 3–4 | Close enough that the arrangement is worth revisiting before the next invoice | orange text |
| 5+ | Not independent, followed by the consequences | orange block, white text |

The colour escalates within the existing tokens — there is no red in the palette, so the third step changes the block rather than reaching for a stronger hue. Both orange states clear the 3:1 contrast threshold that applies to the note's 20px weight-500 text (measured 3.15:1 either way). The script sets these inline; it adds no CSS rules.

The note never names the threshold. It is stated once in the section intro, which is where it belongs.

**The 3–4 band is a practical warning, not a legal one.** The statute knows two states: under five, and five or more. The middle band exists because two criteria drift without the client doing anything — the 70% income share depends on what the contractor earns elsewhere, and the 130-day count simply accumulates. At four you are one fact from the line, and one of those facts is not visible to you. Phrase it that way; do not imply the law grades it.
...
This matters more than the code. Numbers here were checked against 2026 sources; one of the three regimes still rests on an assumption a Serbian adviser needs to confirm.
...
| Related parties are aggregated: article 85(1)(17) covers work for "the same client or a party treated as related to the client", and every one of the nine criteria repeats it. Verified but not on the page — few buyers hire one contractor through two of their own entities | statutory text via knjigovodstvene-usluge.rs and Advokat Vuk Vuković; Unija, ideal-racunovodstvo, direkta |
...
| Eco fee (naknada za zaštitu i unapređivanje životne sredine): mandatory for every legal entity and entrepreneur including paušalci; Form 1 filed by 30 April via the local tax administration portal; the municipality then issues a decision and it is paid quarterly; 5,000 / 10,000 / 20,000 RSD by impact band, capped at 0.4% of the previous year's revenue; IT and consulting sit in the lowest band | Paragraf, Minimax, Biznis.rs, ePreduzetnik, Pausalko — all 2026 |
| VAT 20% / 10% — verified, but no longer used since VAT came off the page | PwC, TaxRavens |
| Corporate tax 15% and dividends 15% — verified, but no longer used since the company route came off the page | PwC, TaxRavens |

### Assumptions in the calculator — confirm before publishing

**Flat-rate (paušal).** The monthly amount comes from an individual Tax Administration decision and cannot be derived from income. The second case card now tells the reader to ask for the figure on their contractor's decision, so the assumption is surfaced outside the calculator as well as inside it. The calculator uses `state.pausalMonthly = 443` USD, roughly 45,000 RSD, the middle of a published 30,000–60,000 RSD range for a Belgrade IT or consulting activity code. That range comes from one relocation-advisory source. Treat it as a placeholder and let the user enter their contractor's actual figure.
...
**The company (d.o.o.) route was removed** from the calculator and from the page on 10 August 2026. Three regimes remain: flat-rate, bookkeeping and the self-assessing freelancer. `MODELS` in the script is now three entries, and the `CIT` and `DIVIDEND` constants are gone.

Both claims that survived the removal have since been checked.

**"Only the two entrepreneur routes carry independence-test exposure" — verified.** Article 85(1)(17) names *preduzetnici* and *preduzetnici paušalci*. Advokat Iva Mikulić states the test does not reach legal entities, farmer-entrepreneurs or other natural persons not registered with the APR; attorney.rs says it applies exclusively to entrepreneurs keeping books and lump-sum entrepreneurs; NCR Lawyers say only two groups of entrepreneurs. An unregistered *frilenser* is outside it.

**"Roughly 12% to nearly 40%" — replaced.** That was Tax Advisor Serbia's spread across four models, and with the company gone it contradicted this page's own calculator. Recomputed from the verified rates across $20k–$120k of invoice: **10% to 45%**. The floor is flat-rate near the ceiling (9.0% at $59k); the ceiling is bookkeeping without the personal-salary election (45.1%, capped once profit passes the maximum contribution base). The card now says "roughly 10% to 45%, depending on the route and whether the personal-salary election was filed", which a reader can reproduce by clicking through the tabs.

This figure is our arithmetic from verified inputs rather than a citation. That is deliberate — it has to agree with the calculator on the same page.
...
- "A one-person company is the recognised way out" — an adviser recommendation, not a statutory safe harbour. Moot since the company route came off the page, but worth knowing if it ever returns.
...
- [ ] Eco fee bands — 5,000 / 10,000 / 20,000 RSD; the Uredba adjusts these periodically
...
- **23 elements have no `data-node-id`** — the expanded FAQ answer blocks, three checklist rows, one calculator tab, the hero disclaimer and the CTA button in `screen`. They have no Figma counterpart yet.
...
- **The year-in-the-life section is at 689px of the 716px it has** before the warncard, with five beats. A sixth will not fit, and lengthening any description by more than a line will collide. The fix is three CSS values: `.yearlife` height to auto, drop `overflow: hidden`, and let `.warncard` flow instead of pinning at 1040px.
